...
export default async function XRayPlugin(eleventyConfig, pluginOptions = {}) {
	...
	try {
		if (options?.onlyEnv && process.env[options.onlyEnvName] !== options.onlyEnv) {
			throw new Error(`onlyEnv condition not met (${process.env[options.onlyEnvName]} != ${options.onlyEnv}). Exiting.`);
		}
		else if (eleventyRunMode === 'build' && options.mode === 'serve') {
			throw new Error(`ELEVENTY_RUN_MODE = 'build' but Xray mode = 'serve'. Exiting.`);
		}
	}
	catch(err) {
		debug(err.message);
