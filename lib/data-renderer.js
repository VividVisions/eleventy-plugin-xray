 ...
 * This function (and its dependency) is used in both the Node.js and the 
 * browser version.
 * 
 ...
 * This function is used in both the Node.js and the browser version.
 *
...
export function renderObj(obj, depth = 0) {
	let keyString = (obj?.key) ? escape(obj.key) + ': ' : '';
	...
	if (obj?.where) {
		...
		switch (obj.where) {
			case 'f':
				tooltip = 'Front matter';
				break;
			case 'd':
				tooltip = 'Directory';
				break;
			case 't':
				tooltip = 'Template/Layout';
				break;
		}

		keyString = `<span class="where" title="${tooltip}">${keyString}</span>`;
	}
		...
		case 'object':
			if (obj.content && obj.content.length > 0) {
				...
				<ul class="tree ${ depth > 0 ? ' closed': ''}">
					<label>${keyString}<code class="${obj.type}"><span>${obj.content.length}</span></code></label>
