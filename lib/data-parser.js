 ...
 * Xray parser class to identify and recursively traverse Eleventy data sets.
 *
 * @since 1.0.0
 */
export default class DataParser {
	...
	#crawl(mode, key, object, addTo, seenMap, depth = 0, path = key) {
		...
		else if (depth === 1 && mode === 'page') {
			if (this.dirKeys.includes(key)) {
				data.where = 'd';
			}
			else if (this.layoutKeys.includes(key)) {
				data.where = 't';
			}
			else {
				data.where = 'f';
		...
		if (!isCircular) {
			...
			// Map.
				...
				const sortedKeys = Array.from(object.keys()).sort(sortAlphaNum(this.options.locale));
			...
			// Object.
				...
				const sortedKeys = Object.keys(object).sort(sortAlphaNum(this.options.locale));
